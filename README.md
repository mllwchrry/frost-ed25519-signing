...
This document adapts the secp256k1 [FROST Signing Protocol for BIP340 Signatures][bip-frost-signing-secp] to Ed25519, carrying over that specification's design while producing ordinary Ed25519 signatures ([RFC 8032][rfc8032]) in place of BIP 340 Schnorr signatures. Key generation for FROST signing is out of scope for this document.
...
This document is written from the coordinator's perspective because the key generation methods compatible with this document, a trusted dealer setup and ChillDKG, also involve a central third party. Implementations are therefore likely to reuse the same setup for signing.
...
Once the coordinator has selected the signers, each signer forms the [Session Context](#session-context) that the second communication round operates on: it looks up the selected identifiers in its own threshold info to obtain their public shares, and adds the aggregate nonce and the message.
...
Before signing, the threshold info must pass *ValidateThresholdInfo*, which checks that every signer set selectable from its public shares reproduces the threshold public key.[^prime-order-keys]
...
A signing session requires at least *t* participants and one coordinator. Across the two communication rounds, the coordinator has five responsibilities:

1. Choosing the signers from among the participants.
2. Relaying contributions and session data to and from the signers.
3. Aggregating the signers' nonces and partial signatures.
4. Identifying the malicious signer when a contribution is invalid.
5. Publishing the resulting signature.

How the coordinator chooses the signers is outside the scope of this document.
The selected signers' identifiers and public shares become part of the [Session Context](#session-context) assembled in the second communication round.

Whenever they want to sign a message, the basic order of operations to create a threshold signature is as follows:

**First communication round:**
Each participant runs *NonceGen* to compute their *secnonce* and *pubnonce*.[^nonce-serialization-detail]
Each participant sends their *pubnonce* to the coordinator, who selects the signers, aggregates their *pubnonces* using *NonceAgg* to produce an aggregate nonce, and sends it back to them.[^signer-selection]
...
[^signer-selection]: The coordinator may request *pubnonces* from more than *u* participants, even all *n* of them, and select the signers afterwards, e.g., the first *t* participants that respond. The *pubnonces* of participants that are not selected are simply never aggregated. The robust signing wrapper ROAST[[RRJSS][roast]] relies on exactly this pattern.
...
Participants run *NonceGen* to generate a batch of *pubnonce* values before the message or the signer set[^preprocess-round1] is known, which are stored with the coordinator (e.g., on a centralized server).
...
[^preprocess-round1]: When preprocessing *NonceGen* round, the coordinator can store the *pubnonces* alongside the participants' identifiers and public shares, as these are generated and stored before the signing session begins.
...
| *hash<sub>tag</sub>(x)* | *tagged_hash(FROST3-ed25519-v1/tag, x)* | Computes a 64-byte domain-separated hash of the byte array *x*. The output is *SHA512(SHA512(FROST3-ed25519-v1/tag)\[0:32] \|\| x)*, where *tag* is a UTF-8 encoded string unique to the context (e.g., *nonce*) |
...
- The list of participant public shares *pubshare<sub>0..n-1</sub>*: *n* entries, each either a 32-byte array (a serialized point) or *empty_bytestring*, where *1 ≤ n ≤ 128*
...
- Fail if not *1 ≤ t ≤ n*
- Fail if *point(thresh_pk)* fails
- Let *id<sub>1..w</sub>* be the identifiers *i* with *pubshare<sub>i</sub> ≠ empty_bytestring*, in ascending order
- For *j = 1 .. w*:
  - Let *P<sub>j</sub> = point(pubshare<sub>id<sub>j</sub></sub>)*; fail if that fails
- Fail if *w < t*
- For *j = t+1 .. w*:
...
- For *i = 1..u*:
  ...
  - For *k = 1..u*:
    - If *id<sub>k</sub> ≠ id<sub>i</sub>*:
      - Let *num = num &middot; Scalar(x - id<sub>k</sub>) &ensp;(mod L)*
      - Let *deno = deno &middot; Scalar(id<sub>i</sub> - id<sub>k</sub>) &ensp;(mod L)*
  - *Q = Q + (num &middot; deno<sup>-1</sup>) &middot; P<sub>i</sub>*
- Return *Q*, which may be the identity element

Internal Algorithm *DeriveThreshPubkey(id<sub>1..u</sub>, P<sub>1..u</sub>)*

- Let *Q = DerivePubshareAt(id<sub>1..u</sub>, P<sub>1..u</sub>, -1)*
- Fail if *is_identity(Q)*
- Return *bytes(Q)*
...
- If the optional argument *secshare* is present:
  - Let *rand' = xor_bytes(secshare, hash<sub>aux</sub>(rand)\[0:32])*[^sk-xor-rand]
- Else:
  - Let *rand' = rand*
...
- Let *k<sub>i</sub> = scalar_from_bytes_wide(hash<sub>nonce</sub>(rand' || bytes(1, len(pubshare)) || pubshare || bytes(1, len(thresh_pk)) || thresh_pk || m_prefixed || bytes(4, len(extra_in)) || extra_in || bytes(1, i - 1)))* for *i = 1,2*
...
- The list of participant public shares *pubshare<sub>1..u</sub>*: either *u* 32-byte arrays, each a serialized point, where *pubshare<sub>i</sub>* belongs to the participant with identifier *id<sub>i</sub>*, or the whole list is absent
- The threshold public key *thresh_pk*: a 32-byte array, serialized point
...
> - The session cannot check its key material against *thresh_pk*.
> - *Sign* cannot check the signer's own *secshare* against their public share.

We write "Let *(n, t, id<sub>1..u</sub>, pubshare<sub>1..u</sub>, thresh_pk, aggnonce, m) = session_ctx*" to assign names to the elements of a Session Context.

Algorithm *GetSessionValues(session_ctx)*:

- Let *(n, t, id<sub>1..u</sub>, pubshare<sub>1..u</sub>, thresh_pk, aggnonce, m) = session_ctx*
- Run *ValidateSessionParams(n, t, id<sub>1..u</sub>, pubshare<sub>1..u</sub>, thresh_pk)*; fail if that fails
- Let *ser_ids* = *SerializeIds(id<sub>1..u</sub>)*[^canonical-ids-det-sign]
- Let *b* = *scalar_from_bytes_wide(hash<sub>noncecoef</sub>(bytes(4, u) || ser_ids || aggnonce || thresh_pk || m))*
...
Internal Algorithm *ValidateSessionParams(n, t, id<sub>1..u</sub>, pubshare<sub>1..u</sub>, thresh_pk)*:
...
Algorithm *PartialSigVerify(psig, pubnonce<sub>1..u</sub>, n, t, id<sub>1..u</sub>, pubshare<sub>1..u</sub>, thresh_pk, m, i)*:
...
- Let *session_ctx = (n, t, id<sub>1..u</sub>, pubshare<sub>1..u</sub>, thresh_pk, aggnonce, m)*
...
- Let *(id<sub>1..u</sub>, _, b, _, e) = GetSessionValues(session_ctx)*; fail if that fails
...
Algorithm *DeterministicSign(secshare, my_id, aggothernonce, n, t, id<sub>1..u</sub>, pubshare<sub>1..u</sub>, thresh_pk, m, aux_rand)*:
...
- Inputs:
  ...
  - The threshold number *t* of participants required to issue a signature: an integer with *1 ≤ t ≤ n*
  - The list of participant identifiers *id<sub>1..u</sub>*: *u* distinct integers with *t ≤ u ≤ n*, each with *0 ≤ id<sub>i</sub> ≤ n - 1*
  - The list of participant public shares *pubshare<sub>1..u</sub>*: *u* 32-byte arrays, each a serialized point, or the whole list is absent
  - The threshold public key *thresh_pk*: a 32-byte array, serialized point
  - The message *m*: a byte array[^max-msg-len]
  - The auxiliary randomness *aux_rand*: a 32-byte array (optional argument)
- Run *ValidateSessionParams(n, t, id<sub>1..u</sub>, pubshare<sub>1..u</sub>, thresh_pk)*; fail if that fails
- If the optional argument *aux_rand* is present:
  - Let *secshare' = xor_bytes(secshare, hash<sub>aux</sub>(aux_rand)\[0:32])*
- Else:
...
- Let *k<sub>i</sub> = scalar_from_bytes_wide(hash<sub>deterministic/nonce</sub>(secshare' || bytes(4, my_id) || bytes(4, u) || SerializeIds(id<sub>1..u</sub>) || aggothernonce' || thresh_pk || bytes(8, len(m)) || m || bytes(1, i - 1)))* for *i = 1,2*
- Fail if *k<sub>1</sub> = Scalar(0)* or *k<sub>2</sub> = Scalar(0)*[^negligible-zero-scalar]
- Let *R<sub>\*,1</sub> = k<sub>1</sub> &middot; B, R<sub>\*,2</sub> = k<sub>2</sub> &middot; B*
- Let *pubnonce = bytes(R<sub>\*,1</sub>) || bytes(R<sub>\*,2</sub>)*
